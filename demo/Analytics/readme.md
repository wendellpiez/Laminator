...
Run over *Frankenstein*, this produces a graph with an interesting finding: in the Frankenstein document, apparently `volume` markup overlaps `page` markup. In this case, the apparent oddity is a reflection of the hybrid (monstrous) nature of the markup itself - pages are numbered and marked from the 1831 edition, but only the the 1818 edition came in three volumes (whose boundaries are marked - arguably, misleadingly so, as the anomaly in markup semantics attests).
